...
class EmailRegisterTemplateSpec extends UnitSpec with FakeCCApplication with CCSession {

  implicit val request = FakeRequest()

  "EmailRegistration Template" should {
    ...
    "display errors when the form has errors - dob save not selected" in {
      EmailRegistrationForm.form.bind(Map(
        ...
        errors => {
          val template = views.html.emailRegistration(errors)(request)
          val doc = Jsoup.parse(contentAsString(template))
