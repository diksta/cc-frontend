  ...
  "HowManyChildrenController" when {
    ...
    "GET" should {
      ...
      "not respond with NOT_FOUND" in {
        val result = route(FakeRequest(GET, "/childcare-calculator-qa/children/number"))
    ...
    "POST" should {
      ...
      "redirect when POST is successful with value already in keystore (adding)" in {
        ...
        status(result) shouldBe Status.SEE_OTHER
        result.header.headers.get("Location").get shouldBe "/childcare-calculator-qa/children/details/1"
      }
